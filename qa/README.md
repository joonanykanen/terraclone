...
| `tc_light_qa.js` | Sky-light regression suite (the "dug shaft keeps the top of the screen lit" bug) — 18 checks: digs vertical shafts in a clean (cave/tree-free) column, probes `TC.lightAt` at known depths: shaft air dark ~13 tiles below the original surface, underground view top dark (day + night), surface/sky/stone gradient unchanged, player light intact, FPS holds | `node qa/tc_light_qa.js` |
| `light_harness.js` | Deterministic Node harness: extracts the REAL lighting code from `index.html` and runs it on a synthetic world — used to prove the bug pre-fix (top-of-view stone read 0.87 at 20 m depth; shaft air 1.0 at any depth) and verify the fix with byte-identical tree/canopy values | `node qa/light_harness.js` |
