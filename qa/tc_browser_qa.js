...
(async () => {
  ...
  const scenarios = [
    ...
    ['05_snow_biome', 'return (function(){let x=-1;for(let i=0;i<400;i++){if(TC.biome[i]===2){x=i;break;}}if(x<0)return "NO_SNOW";TC.setGameTime(300*0.25);var p=TC.player;p.x=x*16-6;p.y=TC.surf[x]*16-25;p.vy=0;p.vx=0;p.apex=p.y;p.hp=100;return "col"+x;})();', 3500],
    ['06_desert_biome', 'return (function(){let x=-1;for(let i=0;i<400;i++){if(TC.biome[i]===1){x=i;break;}}if(x<0)return "NO_DESERT";TC.setGameTime(300*0.3);var p=TC.player;p.x=x*16-6;p.y=TC.surf[x]*16-25;p.vy=0;p.vx=0;p.apex=p.y;p.hp=100;return "col"+x;})();', 3500],
